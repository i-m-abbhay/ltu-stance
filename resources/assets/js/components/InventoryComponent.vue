        ...
        .then(
          function (response) {
            ...
            if (response.data) {
              ...
              if (result === "SHELF_ERROR") {
                alert("選択した棚は存在しません。");
              } else {
                //棚QRスキャン
                this.loading = true;
                localStorage.setItem("qrCode", JSON.stringify(this.result));
                localStorage.setItem(
                  "shelf_number_id",
                  JSON.stringify(result.shelf_number_id)
                );
                localStorage.setItem("shelf_area", JSON.stringify(result.shelf_area));
                localStorage.setItem("warehouse_id", JSON.stringify(result.warehouse_id));
                localStorage.setItem(
                  "warehouse_name",
                  JSON.stringify(result.warehouse_name)
                );
