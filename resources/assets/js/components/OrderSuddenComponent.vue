                ...
                <div class="input-area-body area-border">
                    <div class="input-area-body-top">
                        ...
                        <div class="row">
                            <div class="col-md-2">
                                ...
                                <input type="text" class="form-control" v-bind:readonly="true" v-model="matterModel.customer_name">
                            </div>
                            <div class="col-md-2">
                                <label class="control-label">施主名</label>
                                <input type="text" class="form-control" v-bind:readonly="true" v-model="matterModel.owner_name">
                            </div>
                        </div>
                        ...
                        <div v-if="!isOwnStock">
                            <div class="row">
                                <div class="col-md-2">
                                    <label class="control-label">案件郵便番号</label>
                                    <input type="text" class="form-control" v-model="addressModel.zipcode" maxlength="7" @change="getAddress" v-bind:readonly="isReadOnly">
                                </div>
                                <div class="col-md-5">
                                    <label class="control-label">案件住所１</label>
                                    <input type="text" class="form-control" v-model="addressModel.address1" @change="setDeliveryAddress" v-bind:readonly="isReadOnly">
                                </div>
                                <div class="col-md-5">
                                    <label class="control-label">案件住所２</label>
                                    <input type="text" class="form-control" v-model="addressModel.address2" @change="setDeliveryAddress" v-bind:readonly="isReadOnly">
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-12">
                    ...
                    <div class="input-area-body-grid">
                        <div class="row tree-grid-operation-area">           
                            <div class="col-md-9 form-inline tree-grid-btn-area">
                            ...
                            <div class="col-md-3 form-inline tree-grid-btn-area">
                                <button type="button" class="btn btn-save " v-on:click="application" v-bind:disabled="(isReadOnly)">発注登録</button>
                                <button type="button" class="btn btn-save " v-on:click="save" v-bind:disabled="(isReadOnly)" v-if="!isOwnStock">一時保存</button>
                            </div>
                        </div>
                        <!-- 階層 -->
...
export default {
    ...
    created() {
        ...
        this.deliveryAddressList = this.pDeliveryAddressList;
        ...
        this.isOwnStock         = this.ownStockFlg === this.FLG_ON;
        this.showMatterDialog   = !this.isOwnStock;
        this.matterModel            = this.pMatterModel;
        this.addressModel            = this.pAddressModel;

        // ロック中判定
        if (this.isOwnStock && this.rmUndefinedBlank(this.lock.id) != '' && this.isOwnLock != this.FLG_ON) {
            this.isLocked = true;
                ...
                switch (col.name) {
                    ...
                    case 'sales_kbn':
                        ...
                        if(
                            ...
                            this.rmUndefinedBlank(row.sales_kbn) !== '')
                        {
                            row.cost_kbn = row.sales_kbn; // 販売区分変更時は仕入区分も同じものに変更する
                            if(this.rmUndefinedZero(row.product_id) !== 0){
...
/*************** 関数 ***************/
        ...
        async changeProduct(product, row, isCode){
            if(product !== null){
                ...
                if(row.product_id !== product.product_id && row.is_cancel !== true){
                    ...
                    // 非同期で取得
                    var productInfo                 = await this.getProductInfo(product.product_id, this.rmUndefinedZero(this.matterModel.customer_id));
                    if(productInfo !== undefined){
                        ...
                        row.quantity_per_case   = this.rmUndefinedZero(productData.quantity_per_case);  // 入り数
                        ...
                        // row.cost_makeup_rate = product.purchase_makeup_rate          // 仕入掛率
                        //row.cost_unit_price = product.normal_purchase_price          // 仕入単価
                        // row.sales_makeup_rate = product.sales_makeup_rate            // 販売掛率
                        ...
                        this.calcTreeGridChangeUnitPrice(row, false);                       // 仕入掛率
                        ...
                        parseInt(this.trimHyphen(product.end_date)) < this.todayYYYYMMDD){
        ...
        async changeCostSalesKbn(row){
            // 非同期で取得
            var unitPriceInfo           = await this.getUnitPrice(row.product_id, this.rmUndefinedZero(this.matterModel.customer_id));
        ...
        getGridLayout () {
            ...
            return [
                ...
                { cells: [{ name: 'sales_use_flg', binding: 'sales_use_flg', header: '階層販売金額利用', wordWrap: true, width: 70, isReadOnly: false, isRequired: false }] },
                { cells: [{ name: 'product_auto_flg', binding: 'product_auto_flg', header: '1回限り登録', wordWrap: true, width: 70, isReadOnly: false, isRequired: false, visible: visibleProductInfoFlg }] },
                { cells: [{ name: 'product_definitive_flg', binding: 'product_definitive_flg', header: '本登録', wordWrap: true, width: 70, isReadOnly: false, isRequired: false, visible: visibleProductInfoFlg }] },
                { cells: [
            ...
            .then( function (response) {
                ...
                if (response.data) {
                    ...
                    if (response.data.invalid) {
                        // 同じ案件があればエラー
                        this.errors.matter_no = response.data.err_message;
                    }else{
                        ...
                        //this.$set(this.matterModel, 'matter_name', response.data.matter_name);
...
/*************** オートコンプリート ***************/
        ...
        selectHeadSupplier: function(sender) {
            ...
            if(item !== null){
                ...
                this.getDeliveryAddressList(item['supplier_id'], this.matterModel.address_id);
                ...
                this.getDeliveryAddressList(null, this.matterModel.address_id);
            }
        },
        ...
        selectDeliveryAddress: function(sender){
            ...
            if(this.tmpDeliveryAddressUniqueKey === ''){
                if (item !== null) {
                    if(item.delivery_address_kbn === this.deliveryAddressKbnList.val.site){
                        // 直送
                        this.main.delivery_address = this.addressModel.address1 + this.addressModel.address2;
                    }else{
...
/*************** 日付 終了 ***************/
        ...
        save(){
            ...
            for(var i=0;i<gridDataList.length;i++){
                // 一式の発注は1個のみ
                if(!this.treeGridSetProductSaveIsValid(gridDataList[i], gridDataList[i].order_quantity)){
            ...
            params.append('grid_all_data', JSON.stringify(this.wjMultiRowControle.collectionView.sourceCollection));
            params.append('cost_total', 0);
            params.append('sales_total', 0);
            params.append('profit_total', 0);

            axios.post('/order-sudden/save', params, {headers: {'Content-Type': 'multipart/form-data'}})
            .then( function (response) {
                this.loading = false
                if (response.data) {
                    if (response.data.status) {
                        ...
                        if(this.isOwnStock){
                            ...
                            window.onbeforeunload = null;
                            location.href = '/order-edit/' + response.data.matterId;
                        }
                    }else{
        ...
        application(){
                    ...
                    if(this.rmUndefinedZero(this.main.maker_id) !== 0){
                        if(gridDataList[i].maker_id == this.main.maker_id){
                            if(this.bigNumberGte(this.rmUndefinedZero(gridDataList[i].order_quantity), 0.01)){
                                if(this.isOwnStock && this.rmUndefinedZero(gridDataList[i].product_id) === 0){
                                    // 当社在庫品の場合、商品マスタにある商品のみ発注可能
                                    errMsg = MSG_ERROR_NOT_PRODUCT_MASTER_DATA_ORDER;
                                    break;
                                }
                                if(this.isOwnStock && this.rmUndefinedZero(gridDataList[i].intangible_flg) === this.FLG_ON){
                                    // 当社在庫品の場合、無形品は発注できない
            ...
            axios.post('/order-sudden/application', params, {headers: {'Content-Type': 'multipart/form-data'}})
            .then( function (response) {
                this.loading = false
                if (response.data) {
                    if (response.data.status) {
        ...
        getOrderSaveData(){
            ...
            params.append('own_stock_flg', this.rmUndefinedZero(this.ownStockFlg));

            params.append('zipcode', this.rmUndefinedBlank(this.addressModel.zipcode));
            params.append('address1', this.rmUndefinedBlank(this.addressModel.address1));
            params.append('address2', this.rmUndefinedBlank(this.addressModel.address2));

            params.append('matter_id', this.rmUndefinedZero(this.matterModel.id));
            params.append('customer_id', this.rmUndefinedZero(this.matterModel.customer_id));
            //params.append('customer_name', this.rmUndefinedBlank(this.matterModel.customer_name));
            params.append('owner_name', this.rmUndefinedBlank(this.matterModel.owner_name));
            params.append('architecture_type', this.rmUndefinedBlank(this.matterModel.architecture_type));
            params.append('matter_name', this.rmUndefinedBlank(this.matterModel.matter_name));
            params.append('department_id', this.rmUndefinedZero(this.matterModel.department_id));
            params.append('staff_id', this.rmUndefinedZero(this.matterModel.staff_id));
        ...
        getDeliveryAddressList(supplierId, matterAddressId){
            ...
            params.append('own_stock_flg', this.ownStockFlg);
            axios.post('/order-sudden/get-delivery-address-list', params)
            .then( function (response) {
        ...
        // 郵便番号から住所自動入力
        getAddress() {
            var zipcode = this.rmInvalidNumZero(this.addressModel.zipcode);
            new YubinBango.Core(zipcode, function(addr){
                var addr1 = addr.region + addr.locality + addr.street
                this.addressModel.address1 = addr1
                this.setDeliveryAddress();
            }.bind(this))
        },
        // 入荷先を直送にしていた場合にラベルにセットする
        setDeliveryAddress() {
            if(this.main.delivery_address_kbn === this.deliveryAddressKbnList.val.site){
                this.main.delivery_address = this.addressModel.address1 + this.addressModel.address2;
            }
        },
