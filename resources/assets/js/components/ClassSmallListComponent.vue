                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            ...
                            <wj-auto-complete class="form-control"
                                ...
                                selected-value-path="class_small_name"
                                :selected-value="searchParams.class_small_name"
                                :isDisabled="isReadOnly"
                                :isRequired=false
                                :initialized="initSmallName"
                                :selectedIndexChanged="changeIdxSmall"
                                :max-items="classSmallList.length"
                            ></wj-auto-complete>
                        </div>
                        <div class="col-md-3">
                            <label class="control-label">工事区分名</label>
                            <wj-auto-complete class="form-control"
                    ...
                    <div class="row">
                        <div class="col-md-12" style="padding-top: 20px;">        
                            <div class="pull-right">                    
                                ...
                                &emsp;
                                <a type="button" class="btn btn-search btn-md" href="/class-small-edit/new">新規作成</a>
                                &emsp;
...
export default {
    ...
    mounted: function() {     
        ...
        this.$nextTick(function() {
            this.wjSmallGrid = this.createGridCtrl('#wjSmallGrid', new wjCore.CollectionView([]));
        });
    },
    methods:{
        changeIdxSmall: function(sender){
            var tmpConst = this.constructionList;
            if (sender.selectedValue) {
                var item = sender.selectedItem;
                tmpConst = [];
                for(var key in this.constructionList) {
                    if (item.construction_id == this.constructionList[key].id) {
                        tmpConst.push(this.constructionList[key]);
                    }
                }
            }
            this.wjSearchObj.construction_name.itemsSource = tmpConst;
            this.wjSearchObj.construction_name.selectedIndex = -1;
        },
        search() {
            this.loading = true;
            ...
            params.append('class_small_name', this.rmUndefinedBlank(this.wjSearchObj.class_small_name.text));
            params.append('construction_name', this.rmUndefinedBlank(this.wjSearchObj.construction_name.text));
            
            axios.post('/class-small-list/search', params)
            .then( function (response) {
                if (response.data) {
                    // URLパラメータ作成
                    this.urlparam = '?'
                    this.urlparam += 'id=' + encodeURIComponent(this.rmUndefinedBlank(this.searchParams.id));
                    this.urlparam += '&' + 'class_small_name=' + encodeURIComponent(this.rmUndefinedBlank(this.wjSearchObj.class_small_name.text));
                    ...
                    response.data.forEach(element => {
                        ...
                        this.keepDOM[element.id].edit_btn.href = '/class-small-edit/' + element.id + this.urlparam + '&mode=2';
                        this.keepDOM[element.id].edit_btn.classList.add('btn', 'btn-md', 'btn-edit', 'edit-link');

                        itemsSource.push({
                            id: element.id,
                            class_small_name: element.class_small_name,
                    ...
                    this.wjSmallGrid.dispose();

                    this.wjSmallGrid = this.createGridCtrl('#wjSmallGrid', new wjCore.CollectionView(itemsSource));

                    // this.wjSmallGrid.itemsSource = itemsSource;
                    this.tableData = dataLength;

                    // 描画更新
                    this.wjSmallGrid.refresh();
                }
        ...
        createGridCtrl(targetGridDivId, gridItemSource) {
            ...
            gridCtrl.itemFormatter = function(panel, r, c, cell) {
                ...
                if (panel.cellType == wjGrid.CellType.Cell) {
                    ...
                    var colName = this.wjSmallGrid.getBindingColumn(panel, r, c).name;
            ...
            gridCtrl.cellEditEnded.addHandler(function(s, e) {
                ...
                // 編集したカラムを特定
                var col = this.wjSmallGrid.getBindingColumn(e.panel, e.row, e.col);       // 取れるのは上段の列名
                // 編集行データ取得
        ...
        getLayout() {
            return [
                ...
                {
                    cells: [
                        { binding: 'construction_name', name: 'construction_name', header: '工事区分名', minWidth: GRID_COL_MIN_WIDTH, width: 380, isReadOnly: true },
                    ]
                },
                {
                    cells: [
                        { binding: 'class_small_name', name: 'class_small_name', header: '工程名称', minWidth: GRID_COL_MIN_WIDTH, width: 380, isReadOnly: true },
                    ]
                },                
                {
                    cells: [
        ...
        // 以下オートコンプリートの値取得
        initSmallName: function(sender) {
            this.wjSearchObj.class_small_name = sender
        },
