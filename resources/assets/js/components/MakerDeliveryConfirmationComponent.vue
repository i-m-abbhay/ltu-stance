  ...
  <div class>
    ...
    <div class="grid-form">
      ...
      >
        ...
        <el-table-column label="メーカー名" width="150">
          <template slot-scope="scope">{{scope.row.supplier_name}}</template>
        </el-table-column>

        <el-table-column label="数量" width="100">
          <template slot-scope="scope">{{scope.row.return_quantity}}</template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <div class="col-md-12 text-center">
      ...
      <button class="btn btn-primary" v-bind:disabled="isSaved" @click="process">引渡完了</button>
    </div>
  </div>
</template>
  ...
  methods: {
    checkTable: function() {
      return this.tableData.filter(
        row => row.return_quantity == row.input_quantity && row.input_quantity != 0
      );
    },
    logErrors(promise) {
    ...
    process() {
      ...
      this.isSaved = true;
      axios
        .post("/maker-delivery-confirmation/save", {
    ...
    //サイン
    clickSign() {
      var listUrl = "/maker-delivery-sign";
