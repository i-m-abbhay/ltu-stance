...
class AlertOverTrust extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'batch:alert-over-trust {setDate? : 指定日}';
     ...
     * @var string
     */
    protected $description = '受注額が与信額を超える場合、通知データを作成';
    ...
    public function __construct()
    {
        ...
        $log_path =  storage_path() .'/logs/batch/alert-over-trust.log'; //出力ファイルパス
    ...
    public function handle()
    {
        ...
        try {
            ...
            // タイムスタンプ用
            $nowDate = Carbon::now();
            $this->log->addInfo("与信オーバーアラートバッチ実行",['dateTime' => $nowDate]);
            ...
            $this->log->addInfo("与信オーバーアラート通知実行完了,".$num."件作成");
        } catch (\Exception $e) {
            DB::rollBack();
            $send = new SendToChatwork();
            $send->sendMessage("バッチ名：alert-over-trust".PHP_EOL."Message：".$e->getMessage().PHP_EOL."Line：".$e->getLine(),"与信オーバーバッチ実行失敗");
            $this->log->error('与信オーバーアラート通知実行失敗', ['error' => $e]);
        }
    }
    ...
    private function searchTarget($executeDate)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if(!empty($data[$key]->sales_total)){
                // ⓹⓸の金額と➀で取得した与信額を比較し、⓸の販売金額が大きい場合アラートを送付する
                if($data[$key]->notPaymentAmount + $data[$key]->sales_total[0]->sales_total > $value->line_of_credit){
                    $noticeList[] = $value;
                }
            }
        }
        ...
        if(!empty($noticeList)){
            foreach ($noticeList as $key => $value) {
                ...
                ->select([
                    ...
                    DB::raw("CONCAT('【与信超過】',  M1.customer_name) as content"),
